# How to use a React Bryntum Calendar with .NET and SQLite

[Bryntum Calendar](https://bryntum.com/products/calendar/) is a performant, highly customizable JavaScript UI component 
with multiple views. It integrates with the major JavaScript web frameworks, including React. This tutorial demonstrates 
how to use the React Bryntum Calendar with a [.NET Framework](https://dotnet.microsoft.com/en-us/) backend and SQLite. 
...
- Configure the Bryntum Calendar to load data from the database and synchronize changes to the database using 
  the created API endpoints.

Here's what we'll build:

![ReactBryntum Calendar](images/bryntum-calendar-complete.png)
...
the completed tutorial is in the [`completed-app`](https://github.com/ritza-co/bryntum-calendar-dotnet-starter/tree/completed-app) 
...
Clone the [Bryntum React Calendar starter GitHub repository](https://github.com/ritza-co/bryntum-calendar-react-starter). 
The code for the completed tutorial is in the [`completed-app`](https://github.com/ritza-co/bryntum-calendar-react-starter/tree/completed-app) 
branch of the repository.
...
In Bryntum Calendar, data stores are kept and linked together in the [project](https://bryntum.com/products/calendar/docs/guide/Calendar/data/displayingdata#the-calendar-project). 
...
We define the `Event` model class that represents the `"events"` table in the database. The table name is set using 
the `[Table("events")]` attribute.
...
The `SyncRequest` class contains the `requestId` and the optional `events` and `resources` properties that hold 
the changes for each store. The `StoreChanges<T>` generic class contains lists for added, updated, and removed records. 
...
record. You should not persist phantom identifiers in your database.
</div>

## Create the database context

Create a folder called `Data` in the project directory. In this folder, create a `CalendarContext.cs` file containing 
...
Now let's update the `Program.cs` file to configure the .NET backend to use SQLite and create a seeding function 
that populates a local SQLite database with the example JSON data from the `example-data` directory.
...
Create a folder called `Controllers` in the project directory. Create a file called `CalendarController.cs` in this 
folder. First, add the controller class with the `/api/load` endpoint:
...
In `CalendarController.cs`, add the following `ApplyEventChanges` method below the `Sync` method to handle 
...
appropriate database operation. For added records, we return the phantom ID and the created database ID.
...
`id` of `"app"`. The `date` property sets the initial date to display, which is **July 20, 2026**, 
to match the example data.
...
Update the `App.tsx` file to import and render the React Bryntum Calendar:
...
We pass in our configuration to the `BryntumCalendar` component and reference the `calendar` ref to access the Calendar 
...
Open [http://localhost:5173](http://localhost:5173/) in your browser. You'll see a Bryntum Calendar with the example 
data from the local SQLite database:

![React Bryntum Calendar with CRUD functionality](images/bryntum-calendar-crud.webm)

Because the React Bryntum Calendar has CRUD functionality, any changes made to it are saved to the SQLite database.

## Next steps

This tutorial covers the basics of using a React Bryntum Calendar with .NET and SQLite. Take a look at the 
[Bryntum Calendar examples page](https://bryntum.com/products/calendar/examples/) to browse the additional features 
you can add to your Calendar, such as these React-specific ones:

- [Dragging items from an external grid onto tasks](https://bryntum.com/products/calendar/examples/frameworks/react-vite/drag-onto-tasks/)
- [Using React renderers](https://bryntum.com/products/calendar/examples/frameworks/react-vite/rendering/)
- [Multi-resource scheduling modes](https://bryntum.com/products/calendar/examples/frameworks/react-vite/resource-modes/)