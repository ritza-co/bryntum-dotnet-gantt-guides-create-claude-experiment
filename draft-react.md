...
project management and scheduling. It integrates with the major JavaScript web frameworks, including React. This
tutorial demonstrates how to use the React Bryntum Gantt with a [.NET Framework](https://dotnet.microsoft.com/en-us/)
backend and SQLite.
...
![React Bryntum Gantt](images/bryntum-gantt-complete.png)
...
Clone the [Bryntum React Gantt starter GitHub repository](https://github.com/ritza-co/bryntum-gantt-react-starter). The
code for the completed tutorial is in the
[`completed-app`](https://github.com/ritza-co/bryntum-gantt-react-starter/tree/completed-app) branch of the repository.
...
We create a configuration object for the Bryntum Gantt. The `viewPreset` property sets the time axis granularity to show
weeks and day letters. The `barMargin` sets spacing between task bars.
...
Update the `App.tsx` file to import and render the React Bryntum Gantt:
...
We pass in our configuration to the `BryntumGantt` component. The `gantt` ref provides access to the Gantt instance for
...
Because the React Bryntum Gantt has CRUD functionality, any changes made to it are saved to the SQLite database.
...
- [Resource histogram](https://bryntum.com/products/gantt/examples/frameworks/react-vite/resource-histogram/)
